@extends('layouts.app-master', ['activePage' => 'product',  'activeSub'=> 'sub_sub_cat',   'titlePage' => __('Edit Sub Sub Category')])
            ...
            <div class="col-md-8 col-lg-10 offset-1">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route('sub_sub_category.update', $subsubCategory) }}" method="post" enctype="multipart/form-data">
                            @method('put')
                            @csrf
                            <div class="form-group">
                                <h5>Sub Sub-Category Name<span class="text-danger">*</span></h5>
                                <div class="controls">
                                    <input type="text" value="{{ old('subsubcategory_name', $subsubCategory->subsubcategory_name) }}" name="subsubcategory_name" class="form-control" required="" data-validation-required-message="This field is required"> <div class="help-block"></div>
                                </div>
                                ...
                                <div class="col-lg-5 col-md-6 col-sm-3">
                                    <select name="subcategory_id" class="custom-select" data-size="7" data-style="btn btn-success btn-round" >
                                        ...
                                        @foreach ($subcategories as $subsub)
                                            <option value="{{ $subsub->id }}" {{ $subsub->id == $subsubCategory->subcategory_id ? 'selected':'' }}>{{ $subsub->subcategory_name }}</option>
                                        @endforeach
                                    </select>
                                </div>
